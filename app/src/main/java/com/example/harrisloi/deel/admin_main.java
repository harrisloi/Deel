...
public class admin_main extends AppCompatActivity
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        fab.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                startActivity(new Intent(admin_main.this, AdminCreateClient.class));
            }
        });
    ...
    private void loadClient() {
        adapter = new FirebaseRecyclerAdapter<User, ClientViewHolder>(User.class, R.layout.client_layout, ClientViewHolder.class, users.orderByChild("role").equalTo("customer") ) {
            @Override
            protected void populateViewHolder(ClientViewHolder viewHolder,final User model,final int position) {
                viewHolder.clientName.setText(model.getFirstName()+" "+ model.getLastName());
                viewHolder.clientEmail.setText(model.getEmail());
                viewHolder.clientPhone.setText(model.getPhoneNumber());
    ...
    private void showEditDialog(final String key, final User model) {
        AlertDialog.Builder alertDialog = new AlertDialog.Builder(admin_main.this);
    ...
    private void updateUser(User model) {
        if(!edtEmail.getText().toString().equals(null) && !edtEmail.getText().toString().equals("") && !edtPhoneNumber.getText().toString().equals(null) && !edtPhoneNumber.getText().toString().equals("")
            ...
            users.child(AccessKey).updateChildren(update)
                    .addOnSuccessListener(new OnSuccessListener<Void>() {
                        @Override
                        public void onSuccess(Void aVoid) {
                            Toast.makeText( admin_main.this,"Updated User successfully.", Toast.LENGTH_SHORT).show();
                        }
                    }).addOnFailureListener(new OnFailureListener() {
                @Override
                public void onFailure(@NonNull Exception e) {
                    Toast.makeText( admin_main.this,"Updated User " + e, Toast.LENGTH_SHORT).show();
                }
            });

        }
        else{
            Toast.makeText(admin_main.this,"Please make sure to fill all the information.", Toast.LENGTH_SHORT).show();
        }
    }
    ...
    private void showRemoveDialog(final String key) {
            ...
            public void onClick(DialogInterface dialog, int which) {
                switch (which) {
                    case DialogInterface.BUTTON_POSITIVE:
                        ...
                        Toast.makeText(admin_main.this, "Shipper has been deleted.F", Toast.LENGTH_SHORT).show();
                        break;
        ...
        builder.setMessage("Delete client?").setPositiveButton("Yes", dialogClickListener)
    ...
    public boolean onNavigationItemSelected(MenuItem item) {
        ...
        if (id == R.id.nav_client) {
            startActivity(new Intent(admin_main.this, admin_main.class));
        } else if (id == R.id.nav_vendor) {
            startActivity(new Intent(admin_main.this, AdminVendor.class));
        } else if (id == R.id.nav_shipper) {
            startActivity(new Intent(admin_main.this, AdminShipper.class));
        } else if (id == R.id.nav_history) {
            startActivity(new Intent(admin_main.this, AdminHistory.class));
        } else if (id == R.id.nav_Logout) {
            Intent SignOut = new Intent(admin_main.this, MainActivity.class);
            SignOut.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK | Intent.FLAG_ACTIVITY_CLEAR_TASK);
            SharedPreferences settings = getSharedPreferences("UserInfo", 0);
            settings.edit().clear().commit();
            startActivity(SignOut);
            Toast.makeText(admin_main.this, "Sign out successfully.", Toast.LENGTH_SHORT).show();
        }
